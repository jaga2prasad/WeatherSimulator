...
public class GeographyRules implements IWeatherRule{
	...
	private WeatherModel getTemperature(WeatherModel input)  throws Exception{
		...
		Map<String, Hemisphere> HemisphereMap = new HashMap<String, Hemisphere>();
			...
			//get Hemisphere
			hemisphere = HemisphereMap.get(latitude >= 0? WeatherUtil.NORTHERNHEMISPHERE: WeatherUtil.SOUTHERNHEMISPHERE);
			...
			//if(input.getTime().get(Calendar.AM_PM) == Calendar.PM)temperature+=-1;
	...
	{
		return (Math.abs(g1.getDegrees() - latitude)<=
		 Math.abs(g2.getDegrees() - latitude))?g1:g2;
